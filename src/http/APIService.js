...
const API_URL = 'https://leotorres.pythonanywhere.com/'; /* 'http://localhost:8000' ‘http://127.0.0.1:8000’ or 'https://yourPythonAnywhereName.pythonanywhere.com'*/
  ...
  export class APIService {
   ...
   getStock(paramPk) {
    const url = `${API_URL}/api/stocks/${paramPk}`;
    let jwtToken = localStorage.getItem('access');
    const headers = { Authorization: `JWT ${jwtToken}` };
    return axios.get(url, { headers: headers });
  }
  ...
  deleteStock(stockPk) {
    const url = `${API_URL}/api/stocks/${stockPk}`;
    let jwtToken = localStorage.getItem('access');
    const headers = { Authorization: `JWT ${jwtToken}` };
    return axios.delete(url, { headers: headers });
  }
